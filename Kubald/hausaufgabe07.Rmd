...
for(s in c(3,4,5) ){
  ...
  output <- paste("Bei einer Standardabweichung von", s, "bestehen", ausgezeichnet*100, "% ausgezeichnet")
  print(output)}
```

Wie steht die Anzahl guter Noten in Beziehung zur Anzahl schlechter Noten? 
Im Vergleich ist der Anteil der Personen, die mit einer guten Note abschließen geringer als der Anteil der Personen, die durchfallen.

Warum?

Da für die Berechnung der Personen, die durchfallen, mehr Notenwerte einbezogen werden (von 0-5) als für die Berechnung der ausgezeichnet abschließenden Personen (13-15)
...
``` {r}
zt.n50 <- sqrt(50) * ((10 - 8)/3)
print(zt.n50)
```

Das ist ein **signifikanter** Unterschied, da 4.71 > 1.88
